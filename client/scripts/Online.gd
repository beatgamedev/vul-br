...
func host(port: int = 4444) -> Error:
	...
	return OK
func join(address: String = "127.0.0.1", port: int = 4444) -> Error:
	var peer: ENetMultiplayerPeer = ENetMultiplayerPeer.new()
...
func _player_setup(display_name: String) -> void:
	var peer_id: int = multiplayer.get_remote_sender_id()
	var player: LobbyPlayer = players.get(peer_id)
...
func _player_connected(peer_id: int) -> void:
	var player: LobbyPlayer = preload("res://prefabs/Player.tscn").instantiate()
...
func _player_removed(peer_id: int) -> void:
	var player: LobbyPlayer = players.get(peer_id)
	if player:
