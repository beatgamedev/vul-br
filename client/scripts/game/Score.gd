...
var score: int:
	get:
		var combo_progress: float = float(_combo_score) / float(_max_combo_score)
		var accuracy_progress: float = float(_base_score) / float(_max_base_score)
		return (
...
func flush() -> void:
	...
	_is_score_changed = false
	_is_multiplier_changed = false
	_is_combo_changed = false
	_is_health_changed = false
